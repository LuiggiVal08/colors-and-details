...
import { PaymentMethod, UpdatePaymentMethodDTO } from '@/types/paymentMethod';
...
export const EditPaymentMethodForm = ({ initialData, onClose, onSuccess }: Props) => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: UpdatePaymentMethodDTO) => paymentMethodService.update(initialData.id, data),
    onSuccess: () => {
        ...
        <View className="mb-6">
          ...
          <View className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-primary-dark">
            ...
            <View className="flex-row items-center justify-between py-2">
              <Text className="text-base text-slate-700">Activo</Text>
              <Controller
                control={control}
                name="activo"
